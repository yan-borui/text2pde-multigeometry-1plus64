...
def materialize_config(
    ...
    """Bind the fixed experiment template to one data package and result root."""
    config = copy.deepcopy(template)
    prepared_dir = prepared_dir.resolve()
    result_root = result_root.resolve()

    dataset = config["data"]["dataset"]
    dataset["train_manifest"] = str(prepared_dir / "train_windows.json")
    dataset["validation_manifest"] = str(
        prepared_dir / "validation_monitor_windows.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bind a Text2PDE experiment template to portable data and output paths."
    )
