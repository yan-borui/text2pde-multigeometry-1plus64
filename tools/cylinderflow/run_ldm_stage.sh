...
"$python_bin" -m tools.cylinderflow.evaluate_rollout \
  ...
  --manifest "$prepared_dir/validation_full_rollout64.json" \
...
"$python_bin" -m tools.cylinderflow.finalize_validation \
  --result-root "$result_root" \
  > "$result_root/logs/validation_lock.json"
printf 'Validation complete. Test remains sealed; use run_test_stage.sh after human approval.\n'
