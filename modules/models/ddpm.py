...
class DDPM(L.LightningModule):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool):
        ...
        if clip_denoised:
            x_recon.clamp_(-1.0, 1.0)  # clamp the latent space to -1, 1
    ...
    def sample(self, batch_size=1, return_intermediates=False):
        ...
        return self.p_sample_loop(
            (batch_size, channels, image_size[0], image_size[1], image_size[2]),
...
class LatentDiffusion(DDPM):
    ...
    def instantiate_first_stage(self, config):
        ...
        if "gno_coord_dim" in aeconfig["encoder"].keys():  # using mesh ae
            ...
            if "loss" in lossconfig.keys():
                from modules.models.ae.ae_mesh import (
                    Autoencoder,
                )  # use with discriminator
                ...
                model = AutoencoderKL(
                    aeconfig, lossconfig, trainconfig, normalizer=self.normalizer
                )
        else:
            self.mode = "grid"
            if "loss" in lossconfig.keys():
        ...
        if pretrained_path is not None:
            ...
            print(
                f"Autoencoder model loaded from path: {pretrained_path} onto device: {self.device}"
            )
            ...
            if self.use_embed:
                if not self.cond_stage_trainable:
                    ...
                    if os.path.isdir(cache_path):
                        ...
                        self.cond_stage_model = RobertaModel.from_pretrained(
                            cache_path, local_files_only=True
                        )
                    else:
                        self.tokenizer = AutoTokenizer.from_pretrained(
    ...
    def get_learned_conditioning(self, c):
        if self.use_embed:
            ...
            tokens = self.tokenizer(
                c, return_tensors="pt", padding=True
            )  # pads to longest sequence in batch
            tokens = {
                k: v.to(self.device) for k, v in tokens.items()
            }  # manually move to device
            embedding_mask = tokens[
                "attention_mask"
            ]  # 1 where token is not padding, 0 where token is padding
            embedding = self.cond_stage_model(**tokens)[
                "last_hidden_state"
            ]  # b, n, d_embed
            ...
            if self.mode == "mesh":
                ...
                c = self.cond_stage_model(
                    x, pos, self.cond_stage_model.latent_grid, pad_mask
                )
            else:
                x, cond = c
    ...
    def get_input(
        ...
        encoder_posterior = self.encode_first_stage(
            x, pos, pad_mask=pad_mask, cond=cond
        )  # encode x to posterior
        z = self.get_first_stage_encoding(
            encoder_posterior
        ).detach()  # sample from posterior
            ...
            if self.use_embed:  # use text embeddings
                ...
                if self.mode == "grid":
                    for i in range(len(xc)):
                        ...
                        xc[i] = (
                            prepend + xc[i]
                        )  # prepend normalized buoyancy factor to text
                ...
                if self.mode == "mesh":
                    if self.ablate:
                        ...
                        x0 = x[
                            :, :1
                        ]  # get first frame of x as conditioning info (b, 1, n, 3). time dim is eventually flattened so its ok
                    pos0 = pos[
                        :, :1, :, :2
                    ]  # get first frame of pos as conditioning info (b, 1, n, 2), also discard time dim, since trivially 0
                    xc = (x0, pos0, pad_mask)
                else:
                    x0 = x[
                        :, 0
                    ]  # get first frame of x as conditioning info (b, nx, ny, 3)
    ...
    def decode_first_stage(self, z, pos, pad_mask=None, cond=None):
        ...
        if self.mode == "mesh":
            reconstructions = self.first_stage_model.decode(
                z, self.first_stage_model.latent_grid, pos, pad_mask=pad_mask
            )
        else:
    ...
    def encode_first_stage(self, x, pos, pad_mask=None, cond=None):
        if self.mode == "mesh":
            return self.first_stage_model.encode(
                x, pos, self.first_stage_model.latent_grid, pad_mask=pad_mask
            )
        else:
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(
                    c
                )  # if trainable, do a forward pass to get conditioning
        return self.p_losses(x, c, t, *args, **kwargs)

    def apply_model(
        self, x_noisy, t, cond, return_ids=False, cond_scale=None, rescaled_phi=None
    ):
        if cond_scale is not None:
            x_recon = self.model(
                x_noisy, t, cond, cond_scale=cond_scale, rescaled_phi=rescaled_phi
            )
        else:
    ...
    def _get_denoise_row_from_list(
        self, samples, pos, pad_mask=None, cond=None, desc=""
    ):
        denoise_row = []
        for zd in tqdm(samples, desc=desc, leave=False):
            rec = self.decode_first_stage(
                zd.to(self.device), pos, pad_mask=pad_mask, cond=cond
            )
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.learn_sigma:
            B, C = x_noisy.shape[:2]
            assert model_out.shape == (B, C * 2, *x_noisy.shape[2:]), (
                f"model out shape: {model_out.shape}, x_noisy shape: {x_noisy.shape}"
            )
        ...
        if self.learn_sigma:
            ...
            loss_dict.update(
                {f"{log_prefix}/loss_target": loss.clone().detach().mean()}
            )
    ...
    def p_mean_variance(
        ...
        if out is None:
            model_out = self.apply_model(
                x, t_in, c, cond_scale=cond_scale, rescaled_phi=rescaled_phi
            )  # get prediction of noise from model
        else:
            model_out = out
        ...
        if self.learn_sigma:
            ...
            assert model_out.shape == (B, C * 2, *x.shape[2:]), (
                f"model out shape: {model_out.shape}, x shape: {x.shape}"
            )
            model_out, model_var_values = torch.split(model_out, C, dim=1)
    ...
    def log_images(
        ...
        if plot_diffusion_rows:
            ...
            for t in range(self.num_timesteps):
                if t % self.log_every_t == 0 or t == self.num_timesteps - 1:
                    ...
                    z_noisy = self.q_sample(
                        x_start=z_start, t=t, noise=noise
                    )  # gets noised latent z at time t
                    diffusion_row.append(
                        self.decode_first_stage(
                            z_noisy, pos, pad_mask=pad_mask, cond=cond
                        )
                    )

            diffusion_row = torch.stack(diffusion_row)  # n_log_step, T H W C
            log["diffusion_row"] = (
                diffusion_row  # logs forward process from x_0 (data sample) to x_T (fully noised)
            )

        if sample:
            # get denoised row
            samples, z_denoise_row = self.sample_log(
                cond=c, batch_size=N, cond_scale=cond_scale, rescaled_phi=rescaled_phi
            )  # get denoised latent
            x_samples = self.decode_first_stage(
                samples, pos, pad_mask=pad_mask, cond=cond
            )  # decode denoised latent into x_sample
            log["samples"] = x_samples

            # plot intermediate denoised samples
            if plot_denoise_rows:
                denoise_grid = self._get_denoise_row_from_list(
                    z_denoise_row, pos, pad_mask, cond
                )
                log["denoise_row"] = (
                    denoise_grid  # logs reverse process from x_0 (generated sample) to x_T (fully noised)
                )
        ...
        if plot_progressive_rows:
            ...
            prog_row = self._get_denoise_row_from_list(
                progressives, pos, pad_mask, cond
            )
            log["progressive_row"] = (
                prog_row  # logs reverse process from x_T (fully noised) to x_0 (generated sample)
            )
        ...
        if self.scheduler_config is not None:
            if self.scheduler_config["scheduler"] == "cosine":
                ...
                effective_batch_size = (
                    self.scheduler_config["batch_size"]
                    * self.scheduler_config["accumulate_grad_batches"]
                )
                max_epochs = self.scheduler_config["max_epochs"]
                dataset_size = self.scheduler_config["dataset_size"]
                T_max = max_steps or max_epochs * (
                    dataset_size // effective_batch_size + 1
                )
...
class DiffusionWrapper(L.LightningModule):
    ...
    def forward(self, x, t, context=None, cond_scale=None, rescaled_phi=None):
        if self.conditioning_key is None:
            ...
            if cond_scale is not None:
                out = self.diffusion_model.forward_with_cond_scale(
                    x,
                    t,
                    cond_scale=cond_scale,
                    rescaled_phi=rescaled_phi,
